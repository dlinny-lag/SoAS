...
/// <summary>
/// Represents a node in the ComboTreeBox. A node may have a key, text, font style, image and 
...
public class ComboTreeNode : IComparable<ComboTreeNode>, ICloneable {
	...
	/// </summary>
	[Description("The key of the node."), DefaultValue(""), Category("Design")]
	public string Key {
		get { return key; }
		set { key = value; }
	}
	...
	/// <summary>
	/// Initialises a new instance of ComboTreeNode with the specified key and text.
	/// </summary>
	/// <param name="key"></param>
	/// <param name="text"></param>
	public ComboTreeNode(string key, string text) : this() {
		this._text = text;
		this.key = key;
	}

    public ComboTreeNode(string key, string text, object payload) : this(key, text)
    {
        Payload = payload;
    }
	...
	/// Whether to construct the path from the <see cref="Key"/> property 
	...
	public string GetFullPath(string pathSeparator, bool useNodeNamesForPath) {
		...
		s.Append(useNodeNamesForPath ? node.Key : node.Text);

		while ((node = node.Parent) != null) {
			s.Insert(0, pathSeparator);
			s.Insert(0, useNodeNamesForPath ? node.Key : node.Text);
		}
	...
	public override string ToString() {
		if (String.IsNullOrEmpty(key))
			...
			return String.Format("{0} \"{1}\"", key, _text);
	}
